...
namespace WebApi.Dals
{
    public class MTWebApiDAL
    {
        ...
        public ReturnModel<List<RiskManagementAdvPOLMTInfo>> getAdvPOLMTRules(PluginServerInfo Server)
        {
            ...
            string strCount = $"SELECT COUNT(id) AS iCount FROM PluginOrders WHERE MainLableName='{Server.mainLableName}' AND MTType='{Server.mtType}' AND PluginName='{Server.moduleName}' AND IsDelete='N' AND ValidDate>='{DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss")}';";
        ...
        public ReturnModel<string> UploadAdvSwapFeePositions(string MainLableName,string MTType,List<AdvSwapFeePositionInfo> PositionList)
        {
            ...
            try
            {
                ...
                DateTime startTime = TimeZone.CurrentTimeZone.ToUniversalTime(new DateTime(1970, 1, 1, 8, 0, 0, 0));
                PositionList.ForEach(pos =>
                {
                    //DateTime openTime = startTime.AddMilliseconds(pos.OpenTimeStamp-28800000);      //减去8小时，换算成平台时间
                    DateTime openTime = startTime.AddMilliseconds(pos.OpenTimeStamp);
                    lstSql.Add($"INSERT INTO RiskManagement_AdvSwapFeePositions(`Position`,`MainLableName`,`MTType`,`TimeStamp`,`Login`,`Symbol`,`ProfitCurrency`,`Volume`,`Entry`,`StorageMode`,`StorageLong`,`StorageShort`,`PriceOpen`,`PriceCurrent`,`ProfitRate`,`StorageBefore`,`Storage`,`GatewayEntry`,`GatewayVolume`,`GatewayPrice`,`Reason`,`OpenTimeStamp`,`OpenTime`,`ExternalID`,`Comment`,`PriceSL`,`PriceTP`,`Profit`,`PriceMargin`,`UpdateTime`) VALUES({pos.Position},'{MainLableName}','{MTType}',{pos.TimeStamp},{pos.Login},'{pos.Symbol}','{pos.ProfitCurrency}',{pos.Volume},'{pos.Entry}',{pos.StorageMode},{pos.StorageLong},{pos.StorageShort},{pos.PriceOpen},{pos.PriceCurrent},{pos.ProfitRate},{pos.StorageBefore},{pos.Storage},{pos.GatewayEntry},{pos.GatewayVolume},{pos.GatewayPrice},{pos.Reason},{pos.OpenTimeStamp},'{openTime.ToString("yyyy-MM-dd HH:mm:ss:ffff")}','{pos.ExternalID}','{pos.Comment}',{pos.PriceSL},{pos.PriceTP},{pos.Profit},{pos.PriceMargin},'{CurrentTime}') ON DUPLICATE KEY UPDATE `Login`={pos.Login},`Symbol`='{pos.Symbol}',`ProfitCurrency`='{pos.ProfitCurrency}',`Volume`={pos.Volume},`Entry`='{pos.Entry}',`StorageMode`={pos.StorageMode},`StorageLong`={pos.StorageLong},`StorageShort`={pos.StorageShort},`PriceOpen`={pos.PriceOpen},`PriceCurrent`={pos.PriceCurrent},`ProfitRate`={pos.ProfitRate},`StorageBefore`={pos.StorageBefore},`Storage`={pos.Storage},`GatewayEntry`={pos.GatewayEntry},`GatewayVolume`={pos.GatewayVolume},`GatewayPrice`={pos.GatewayPrice},`Reason`={pos.Reason},`OpenTimeStamp`={pos.OpenTimeStamp},`OpenTime`='{openTime.ToString("yyyy-MM-dd HH:mm:ss:ffff")}',`ExternalID`='{pos.ExternalID}',`Comment`='{pos.Comment}',`PriceSL`={pos.PriceSL},`PriceTP`={pos.PriceTP},`Profit`={pos.Profit},`PriceMargin`={pos.PriceMargin},`UpdateTime`='{CurrentTime}'");
                });
